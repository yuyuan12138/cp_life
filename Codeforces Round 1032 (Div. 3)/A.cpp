 ...
 * @Date: 2025-06-17 22:42:26
 * @LastEditors: yuyuan 115848824+yuyuan12138@users.noreply.github.com
 * @LastEditTime: 2025-06-17 22:46:28
 * @FilePath: \Codeforces Round 1032 (Div. 3)\A.cpp
 * @Description: 
 * @
...
void solve(){
    ...
    auto min_num = minv(nums);
    auto max_num = maxv(nums);
    if(s <= min_num){
        cout << max_num - s << endl;
        return ;
    }
    ...
    if(max_num - s >= s - min_num){
        cout << (s - min_num) * 2 + (max_num - s) << endl;
        
    }else{
        cout << (max_num - s) * 2 + (s - min_num) << endl;
    }
    return ;
}
